...
/** Filter out rows where player_id is missing/empty, trim strings */
...
/** Convert "N/A" strings and empty strings to null */
function naToNull(v) {
...
/** Convert Excel serial number or string to ISO date, or null */
function excelDate(v) {
  ...
  return null // not a valid date (e.g. "Recurring Injuries")
}
...
async function importStats(validIds) {
  ...
  const rows = filtered.map(r => ({
    ...
    obpm: num(r.OBPM),
    dbpm: num(r.DBPM),
    rim_fg_pct: num(r['RimFG%']),
    mid_fg_pct: num(r['MidFG%']),
    cs_three_pct: num(r['C&S 3pt%']),
    pu_three_pct: num(r['PU 3pt%']),
    pnr_bh_ppp: num(r['PnR BH PPP']),
    pnr_rm_ppp: num(r['PnR RM PPP']),
    su_ppp: num(r['SU PPP']),
    iso_ppp: num(r['ISO PPP']),
    trans_freq_pct: num(r['Trans freq%']),
    trans_ppp: num(r['Trans PPP']),
  ...
  // Stats uses serial PK, not player_id conflict — use insert (delete first)
  // Delete existing stats for these players to avoid duplicates
  for (const id of validIds) {
    await supabase.from('stats').delete().eq('player_id', id)
  }
  ...
  for (let i = 0; i < rows.length; i += batchSize) {
    ...
    if (error) {
      ...
      console.error('  Sample:', JSON.stringify(batch[0]).slice(0, 300))
    } else {
      inserted += batch.length
    }
  }
...
async function importProspects(validIds) {
  ...
  const rows = filtered.map(r => ({
    ...
    class: str(r.class),
    accolades: str(r.accolades),
    strengths: str(r.strengths),
    weaknesses: str(r.weaknesses),
    comp_upper: str(r.comp_upper),
    comp_lower: str(r.comp_lower),
    tier: str(r.tier),
  }))
...
async function importMeasurables(validIds) {
  ...
  const rows = filtered.map(r => ({
    ...
    max_vertical: num(r.Max_Vertical),
    three_quarter_sprint: num(r['3/4_Sprint']),
    lane_agility: num(r.Lane_Agility),
...
// ---------------------------------------------------------------------------
// 6. RAUS Scores (from RAUS_Auto sheet — importing existing computed values)
// ---------------------------------------------------------------------------
async function importRAUS(validIds) {
  ...
  const rows = filtered.map(r => ({
    ...
    raus_override: num(r.RAUS_Override),
...
// ---------------------------------------------------------------------------
// 7. SSA Scores (from SSA_Auto sheet)
// ---------------------------------------------------------------------------
async function importSSAScores(validIds) {
  ...
  const rows = filtered.map(r => ({
    ...
    creation: num(r['creation ']),  // note trailing space in header
...
// ---------------------------------------------------------------------------
// 8. Athletic Scores (from Measurables sheet — OAI/AAA columns)
// ---------------------------------------------------------------------------
...
async function importAlerts(validIds) {
  ...
  for (const id of validIds) {
    await supabase.from('player_alerts').delete().eq('player_id', id)
  }

  const rows = filtered
    .filter(r => str(r.report_type) || str(r.notes))  // skip rows with no actual alert data
    .map(r => ({
      player_id: str(r.player_id),
...
// ---------------------------------------------------------------------------
async function main() {
  console.log('=== NBA Draft Tracker v2 — Data Import ===')
  ...
  if (error) {
    ...
    console.error('Make sure you have run supabase_schema.sql in your Supabase SQL Editor first!')
    process.exit(1)
  }
  console.log('Supabase connected successfully.')

  const validIds = await importPlayers()
  console.log(`\nImporting data for ${validIds.size} players...\n`)
